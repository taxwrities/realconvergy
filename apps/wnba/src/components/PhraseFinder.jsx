...
export default function PhraseFinder(){
  ...
  /* per-player target toggle (Tony 2026-07-23) — opt-in, default OFF so a
     no-typed-target run stays empty rather than sweeping noise. WNBA wires only
     the opponent-team variant (no "opposing pitcher" analogue). */
  ...
  /* a run is live when there's at least one target source: typed OR the toggle */
  const hasTargets=targets.length>0||oppTeam;
  ...
  const showSrc=oppTeam;

  const hits=useMemo(()=>
    (open&&hasTargets&&selWords.length&&selParts.length&&selCix.length)
      ?findPhrases({words:selWords,parts:selParts,cipherKeys:selCix,targets,tol,oppTeam})
      :[],
    [open,hasTargets,targets,selWords,selParts,selCix,tol,oppTeam,findPhrases]);
    ...
    <div className="finder">
      ...
      {open&&(
        <div className="phrase-body">
          ...
          {/* per-player target toggle (Tony 2026-07-23) — union with the typed
             list above; sweeps every player against THEIR opponent's ciphers. */}
          ...
          {hasTargets&&(
            <div className="id-card" style={{marginTop:6}}>
              <div className="mono muted" style={{fontSize:11.5,marginBottom:4}}>
                targets {[targets.join(', '),oppTeam&&'opp-team'].filter(Boolean).join(' + ')||'—'} · ±{tol} · {hits.length} hit{hits.length===1?'':'s'} across {groups.length} player{groups.length===1?'':'s'}
              </div>
              {groups.map(g=>(
                <ResultRow key={g.id} g={g} tol={tol} showSrc={showSrc}/>
              ))}
              {!hits.length&&(
...
/* target-source badge (Tony 2026-07-23) — credits which target a phrase landed
   ...
   (e.g. "Aces Ord"). A number produced by several sources shows them "·"-joined. */
function srcOne(s){
  if(s.kind==='typed')return'typed';
  if(s.kind==='oppTeam')return`${s.tag} ${s.cipher}`;
  return s.kind||'';
}
function srcBadge(r){
  const ss=r.sources||[];
...
function ResultRow({g,tol,showSrc}){
  ...
  /* multi-source convergence (Tony 2026-07-23): this player has BOTH a typed
     hit AND an opponent-team hit → strong glow. */
  const kinds=new Set(g.rows.flatMap(r=>(r.sources||[]).map(s=>s.kind)));
  const multiSrc=kinds.has('typed')&&kinds.has('oppTeam');
