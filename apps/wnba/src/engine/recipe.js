/* ================================================================
   ...
   The Patterns editor / post sheet stays the source of truth —
   every inferred field is editable. Ported from apps/mlb; the
   opposing-pitcher slot is the opposing starting center in WNBA.
================================================================ */
...
/* board scope tags → grammar scopes: 'season·home'/'season·away' are the
...
/* matchup CROSS row → draft. Only rows anchored on a milestone rung or the
   franchise game # are expressible (name-value = name-value has no counter) —
   the rest return null and get no ⊕ affordance. */
export function draftFromCross(c){
  if(c.rung)return{
    label:`${c.rung.scope} ${c.rung.stat} → ${c.n} = opp center name`,
  ...
  if(c.gameNo)return{
    label:`franchise game #${c.n} = opp center name`,
...
export function draftsToPattern(drafts,laneStat,name='Board recipe'){
  ...
  return{id:'pat-'+Date.now(),name,lane:(first&&statLane[first.slice(5)])||'FB',
