/* ================================================================
   founders — auxiliary founding-date convergence layer (WNBA).

   Port of apps/mlb/src/engine/founders.js. Identical grammar; the only
   sport-specific parts are the category scope and the team table read
   (wnba_teams, never mlb_teams — spec Scope Guardrails: "Never cross
   team tables").
...
/* WNBA scope (spec Scope Guardrails): wnba_teams + the shared ritual /
   institutional / founder / geographic layers. mlb_teams is intentionally
   excluded — never cross sport team tables. */
export const WNBA_FOUNDER_CATEGORIES=[
  'wnba_teams','rituals','secret_societies','institutional',
...
/* founderHitLines — human-readable pieces for ONE decorated hit, used by the
   Board badge popover. Expects the store to have attached `where` (the
   resolved target: DN spine row / loaded active number / 322 anchor — see
   store.founderHits). Returns:
     {value, where, calc, xref}
   value  — the span reading  ("18 years since founding")
   where  — where it landed    ("18 matches DN Row 3" | "prime(18) = 61 matches …")
   calc   — the target's spine formula, when the target is a DN row (else null)
   xref   — the prime-index cross-ref line ("18 = composite · prime(18) = 61"). */
...
/* find the wnba_teams founder record whose name matches a club name. The
   store's team object carries name / teamName / locationName, so accept any
   of them and match on the normalized full name. null when absent. */
export function findTeamFounder(teamFullName){
  const key=normName(teamFullName);
  if(!key)return null;
  return(LOCKED.wnba_teams||[]).find(t=>normName(t.name)===key)||null;
}

/* Board-badge feed: the player's team founding convergences for the day.
...
/* full WNBA-scope probe: every in-scope locked entity decorated with its
...
export function probeFounders(today,activeSet){
  const out=[];
  WNBA_FOUNDER_CATEGORIES.forEach(cat=>{
