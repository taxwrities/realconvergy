...
export const SCOPES=['season','career','both','vsTeam','venue','month','dow'];
...
export function resolveCounter(cond,ctx){
  ...
  if(kind==='rung'){
    ...
    stats.forEach(S=>{
      ...
      /* offset: a fixed rung shift (e.g. PTS+3 → base+3, exact). 0 (default)
    ...
    /* the player's own name (full/first/last) run through ANY of the selected
...
const counterPhrase=(counter,scope,off,arg)=>{
  ...
  if(kind==='rung'){
    ...
    const statName=stat==='GP'?'games-played count':stat;
  ...
  if(kind==='nameCipher'){
    ...
    return`the player's ${arg?.part||'full'} name in ${cips}`;
  }
